...
class OrderedQuestsUI(val page: Int = 1) : InventoryUI {

    override val window by window(SizedWindowOption(5 * 9)) {
        val quests = MariaQuest.playerDataManager.get(player.uniqueId)!!.getQuests()
            .filter { it.status is QuestStatus.InProgress }
            .mapNotNull { MariaQuest.questManager.get(it.id) }

        val maxPage = (quests.lastIndex + (5 * 9)) / (5 * 9)

        title = "${ChatColor.LIGHT_PURPLE}${ChatColor.BOLD}受注しているクエスト"
