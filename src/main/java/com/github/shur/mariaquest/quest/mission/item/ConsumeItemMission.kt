...
class ConsumeItemMission(
    goal: Int,
    onStart: (Player) -> Unit,
    onChangeCount: (Player, before: Int, after: Int) -> Unit,
    onClear: (Player) -> Unit,
    onGiveUp: (Player) -> Unit,
    filter: (Player, ConsumeItemEvent) -> Boolean
) : Mission<ConsumeItemMission.ConsumeItemEvent>(goal, onStart, onChangeCount, onClear, onGiveUp, filter) {

    data class ConsumeItemEvent(val raw: PlayerItemConsumeEvent) {
        val item = raw.item
    }
    ...
    companion object : Listener {
        ...
        fun onConsumeItem(event: PlayerItemConsumeEvent) {
            ...
            playerData.getQuests().forEach { questData ->
                ...
                val mission = quest.missions.getOrNull(status.progress) as? ConsumeItemMission ?: return@forEach

                val missionEvent = ConsumeItemEvent(event)
